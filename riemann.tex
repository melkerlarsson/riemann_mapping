...
\begin{theorem}[Maximumprincipen]
	Låt $\Omega$ vara ett öppet slutet område. Antag att $f \in \H(\Omega)$ och kontinuerlig på $\overline{\Omega}$. Om $f$ inte är konstant så antar $f$ max på $\partial \Omega$.
\end{theorem}
\begin{proof}
	...
	\begin{equation*}
		\abs{f(z_0)} - \abs{f(z_0 + re^{i\theta})} = 0 \iff \abs{f(z_0)} = \abs{f(z)} \quad \forall z \in \Omega.
	\end{equation*}
